...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const isOwnerOrAdmin =
    userRole === "OWNER" || userRole === "ADMIN" || userRole === "MERCHANT";
  ...
  const isNavAuthorized = (navId: CMSView) => {
    ...
    if (userRole === "STOCK_CHECKER") {
      return (
        navId === "products" || navId === "categories" || navId === "dashboard"
      );
    }

    if (userRole === "FULFILLMENT") {
      return (
        navId === "orders" || navId === "shipping" || navId === "dashboard"
      );
    }
    ...
    if (userPermissions) {
      ...
      if (
        navId === "tax" ||
        navId === "payments" ||
        (navId as string) === "payment"
      )
        return !!userPermissions.canManagePayments;
      if (navId === "loyalty")
        return !!userPermissions.canManageCustomers && canUseLoyalty;
