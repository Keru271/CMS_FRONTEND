      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 space-y-4">
          {filteredDiscounts.length === 0 ? (
            <div className="p-8 sm:p-12 rounded-3xl bg-white dark:bg-card border border-dashed border-slate-300 dark:border-border text-center space-y-5 shadow-sm">
              ...
              <div className="space-y-1.5 max-w-md mx-auto">
                <h3 className="font-black text-xl text-slate-900 dark:text-foreground">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                  {searchQuery || activeTab !== 'ALL'
                    ? 'Try adjusting your search filters or create a new promotional campaign.'
                    : 'Launch your first discount campaign to boost store conversions, reward buyers, and increase sales!'}
                </p>
              </div>
              <div className="pt-2 flex justify-center">
                <button
                  type="button"
                  onClick={handleOpenCreateModal}
                  className="px-6 py-3 rounded-2xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-black shadow-lg shadow-indigo-600/30 flex items-center gap-2 transition-all hover:scale-105 active:scale-95"
                >
                  <Plus className="w-4 h-4" />
          ...
          <div className="bg-white dark:bg-card border border-slate-200 dark:border-border rounded-3xl w-full max-w-3xl shadow-2xl overflow-hidden my-8">
            ...
            <form onSubmit={handleSubmitForm} className="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">
                      Applies To Products / Collections
                    </label>
                    <select
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          customerEligibility: e.target.value as DiscountCustomerEligibility,
                        })
                      }
                      className="w-full px-4 py-2.5 rounded-2xl border border-slate-200 dark:border-border bg-slate-50 dark:bg-card text-xs font-bold"
                    >
                ...
                {/* DYNAMIC SELECTION LIST FOR SPECIFIC PRODUCTS */}
                {formData.appliesTo === 'PRODUCTS' && (
                  <div className="p-4 rounded-2xl bg-indigo-50/50 dark:bg-indigo-950/20 border border-indigo-200 dark:border-indigo-900 space-y-3 animate-in fade-in">
                    <div className="flex items-center justify-between">
                      <label className="block text-xs font-black text-indigo-900 dark:text-indigo-300">
                    ...
                    {products.length > 0 ? (
                      <div className="max-h-48 overflow-y-auto space-y-1.5 p-2 rounded-xl bg-white dark:bg-card border border-slate-200 dark:border-border">
                        {products.map((prod) => {
                          const isSelected = formData.targetIds.includes(prod.id);
                          return (
                            <label
                              key={prod.id}
                              className={`flex items-center justify-between p-2.5 rounded-xl border text-xs font-semibold cursor-pointer transition-all ${
                                isSelected
                                  ? 'bg-indigo-50 border-indigo-500 text-indigo-950 dark:bg-indigo-950/60 dark:text-white'
                                  : 'bg-slate-50 dark:bg-accent border-slate-200 dark:border-border text-slate-700 dark:text-slate-300 hover:bg-slate-100'
                              }`}
                            >
                ...
                {/* DYNAMIC SELECTION LIST FOR SPECIFIC COLLECTIONS */}
                    ...
                    {collections.length > 0 ? (
                      <div className="max-h-48 overflow-y-auto space-y-1.5 p-2 rounded-xl bg-white dark:bg-card border border-slate-200 dark:border-border">
                        {collections.map((coll) => {
                          const isSelected = formData.targetIds.includes(coll.id);
                          return (
                            <label
                              key={coll.id}
                              className={`flex items-center justify-between p-2.5 rounded-xl border text-xs font-semibold cursor-pointer transition-all ${
                                isSelected
                                  ? 'bg-amber-50 border-amber-500 text-amber-950 dark:bg-amber-950/60 dark:text-white'
                                  : 'bg-slate-50 dark:bg-accent border-slate-200 dark:border-border text-slate-700 dark:text-slate-300 hover:bg-slate-100'
                              }`}
                            >
                              <div className="flex items-center gap-2.5">
                      ...
                      <div className="text-center py-4 text-xs text-slate-500">
                        No collections available to select. Please create collections first.
                      </div>
                    )}
                  </div>
                )}

                {/* DYNAMIC SELECTION LIST FOR CUSTOMER GROUPS */}
                {formData.customerEligibility === 'GROUPS' && (
                  <div className="p-4 rounded-2xl bg-emerald-50/50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-900 space-y-3 animate-in fade-in">
                    <label className="block text-xs font-black text-emerald-900 dark:text-emerald-300">
                      Select Eligible Customer Groups ({formData.targetCustomers.length} Selected)
                    </label>
                    <div className="grid grid-cols-2 gap-2">
                      {['NEW', 'RETURNING', 'VIP', 'WHOLESALE'].map((grp) => {
                        const isSelected = formData.targetCustomers.includes(grp);
                        return (
                          <label
                            key={grp}
                            className={`flex items-center gap-2 p-2.5 rounded-xl border text-xs font-bold cursor-pointer transition-all ${
                              isSelected
                                ? 'bg-emerald-100 border-emerald-500 text-emerald-900 dark:bg-emerald-900/60 dark:text-white'
                                : 'bg-slate-50 dark:bg-accent border-slate-200 dark:border-border text-slate-700 dark:text-slate-300'
                            }`}
                          >
                            <input
                              type="checkbox"
