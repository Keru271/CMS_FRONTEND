...
export const CMSChatbot: React.FC = () => {
    ...
    return [
      {
        ...
        actions: [
          ...
          },
          {
            ...
            type: "PREFILL_PROMPT",
            payload: "Show unfulfilled orders",
          },
          {
            id: "a4",
            label: "➕ New Product",
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("open-cms-copilot", handleOpenCopilot);
  ...
  const contextualSuggestions =
    aiAssistantService.getContextualSuggestions(pathname);
    ...
    const flushTable = (key: number) => {
      if (tableRows.length > 0) {
        ...
        elements.push(
          <div
            key={`table-${key}`}
            className="my-2.5 overflow-x-auto rounded-lg border border-[#cbd5e0] bg-[#ffffff]"
          >
            <table className="w-full text-left text-xs border-collapse font-sans">
              ...
              <tbody className="divide-y divide-[#cbd5e0]/60">
                {rows.map((r, ri) => (
                  <tr
                    key={ri}
                    className="hover:bg-[#fdf1ef]/40 transition-colors"
                  >
                    {r.map((c, ci) => (
    ...
    lines.forEach((line, index) => {
      ...
      if (trimmed.startsWith("### ")) {
        elements.push(
          <h4
            key={index}
            className="text-xs font-bold text-[#191a1b] mt-3 mb-1 font-sans flex items-center gap-1.5"
          >
        ...
        elements.push(
          <h3
            key={index}
            className="text-sm font-bold text-[#191a1b] mt-3 mb-1 font-sans"
          >
            {parseInlineFormatting(trimmed.replace("## ", ""))}
          </h3>,
        );
        ...
        elements.push(
          <div
            key={index}
            className="flex items-start gap-1.5 my-1 text-xs text-[#191a1b] pl-1 font-sans"
          >
        ...
        if (numMatch) {
          elements.push(
            <div
              key={index}
              className="flex items-start gap-2 my-1 text-xs text-[#191a1b] pl-1 font-sans"
            >
              <span className="text-xs font-bold text-[#5e5a5a] shrink-0">
                {numMatch[1]}.
              </span>
              <div className="flex-1 leading-relaxed">
        ...
        elements.push(
          <p
            key={index}
            className="text-xs text-[#191a1b] leading-relaxed font-sans"
          >
      ...
      {!isOpen && (
        <div className="fixed bottom-6 right-6 z-50 flex items-center gap-2 group">
          ...
          <div className="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#191a1b] text-[#ffffff] text-xs font-medium shadow-lg opacity-0 group-hover:opacity-100 transition-all transform translate-y-1 group-hover:translate-y-0 pointer-events-none">
            ...
            <kbd className="px-1.5 py-0.5 rounded bg-[#ffffff]/20 text-[10px] font-mono">
              ⌘J
            </kbd>
          </div>
            ...
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-1.5">
                  <span className="font-sans font-bold text-xs text-[#ffffff] truncate">
                    Wendr Copilot
                  </span>
                ...
                <p className="text-[10px] text-[#beb9b3] font-sans truncate">
                  {merchantData?.store?.storeName || "Store Assistant"} · Live
                  Context
                </p>
              </div>
            </div>
